...
> **Note:** The examples use version `0.81.0`. This will be the unreleased version if you are at the `main` branch.
...
```bash
helm template my-release oci://ghcr.io/opsta/onechart --version 0.81.0 \
...
```bash
helm install my-release oci://ghcr.io/opsta/onechart --version 0.81.0 \
  --set image.repository=nginx \
  --set image.tag=1.19.3
```
...
```bash
helm template my-release oci://ghcr.io/opsta/onechart --version 0.81.0 -f values.yaml
